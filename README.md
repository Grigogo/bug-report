...
Статусы: **Новая → В работе → На проверку → Готово**. В «Готово» задача попадает только с проверки; с проверки её можно вернуть в работу.

К задаче можно оставлять комментарии (например, тестировщик пишет замечание и возвращает задачу в работу). Комментарии помечаются как «новые» до тех пор, пока задача снова не уйдёт «На проверку» (или не будет принята в «Готово»).
...
- `/tasks/[id]` — карточка задачи (описание, шаги, галерея скриншотов с лайтбоксом, смена статуса)
